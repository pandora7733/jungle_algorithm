...
def find_above_average_students(students):
    ...
    for i in range(len(all_score)):
        score_avg += all_score[i]

        if all_score[-1] == all_score[i]:
            score_avg /= i+1
    ...
    for i in students:
        if i['score'] >= score_avg:
            over_avg_student.append(i["name"])
    
    return score_avg, over_avg_student
